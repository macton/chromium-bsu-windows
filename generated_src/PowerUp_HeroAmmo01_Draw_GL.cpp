...
void Draw_PowerUp_HeroAmmo01()
{
  int live_count     = g_Simulation_PowerUp_HeroAmmo01_Live_Count;
  int live_max_count = kSimulation_PowerUp_HeroAmmo01_Live_MaxCount;
  int update_count   = min( live_count, live_max_count );

  vec2 size = kSimulation_PowerUp_HeroAmmo01_Live_BaseSize;
  for (int index = 0; index < update_count; index++)
  {
    float age = g_Simulation_PowerUp_HeroAmmo01_Live_Age[index];
    if (age > 0.0f)
    {
      vec2  pos = g_Simulation_PowerUp_HeroAmmo01_Live_Pos[index];
